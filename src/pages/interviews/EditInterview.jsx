...
import { fetchInterviewDetail } from "~/services/interviewServices"; // Thêm API để lấy và cập nhật thông tin
import {
  convertToDay,
  convertToHour,
  formatDayFromApi,
...
const EditInterview = () => {
  ...
  useEffect(() => {
    ...
    setSelectedCandidate(
      optionCandidates?.find((c) => c.value === initialValues.candidate.id)
    );
    setSelectedRecruiter(
      optionRecruiters?.find((r) => r.value === initialValues.recruiterDTO.id)
    );
  }, []);
  ...
  const formik = useFormik({
    enableReinitialize: true, // Để sử dụng initialValues khi dữ liệu đã được lấy
  ...
  const test = optionInterviews.filter((interviewer) =>
    formik.values?.interviewerSet.includes((i) => i.id == interviewer.value)
  );
  console.log(">>>test", test);
  // console.log("Check initvalue>>>>", initialValues);
  // console.log("Check>> option candidata", optionCandidates);
                  ...
                  <Col sm={9}>
                    <Select
                      value={optionCandidates?.find(
                        (c) => c.value === initialValues?.candidate.id
                      )}
                      onChange={(selectedOption) => {
                  ...
                  <Col sm={9}>
                    <Select
                      value={optionRecruiters?.find(
                        (r) => r.value === initialValues?.recruiterDTO.id
                      )}
                      onChange={(selectedOption) =>
                        formik.setFieldValue(
                          "recruiterId",
